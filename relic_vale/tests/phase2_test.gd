...
const TEST_SAVE: String="user://phase2-test.json"
...
func run() -> void:
	...
	check(not State.equip("warden_mail"),"Cannot equip an item that is not owned")
	...
	await fight(boss)
	check(State.defeated_unique.has("crypt/warden") and State.inventory.has("warden_mail"),"Final guardian awards unique armor and remains defeated")
	...
	check(State.opened_chests.has(treasure.persistent_id) and State.inventory.has("moon_blade"),"Final treasure chest grants a rare blade and shards")
	coins=State.coins
	treasure.interact(game.player)
	...
	var report: String="# Phase 2 integration verification\n\nGodot %s; renderer %s.\n\n%d passed; %d failed.\n\n" % [Engine.get_version_info().string,DisplayServer.get_name(),checks.size(),failures.size()]
	for entry in checks: report+="- PASS: "+entry+"\n"
	for entry in failures: report+="- FAIL: "+entry+"\n"
	FileAccess.open("res://docs/PHASE_2_TEST_RESULTS.md",FileAccess.WRITE).store_string(report)
	print("PHASE2_RESULT: %d passed, %d failed" % [checks.size(),failures.size()])
