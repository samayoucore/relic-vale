...
$archivePath = Join-Path $workspace 'downloads/RelicVale-v0.8-baseline.zip'
if (Test-Path -LiteralPath $archivePath) { throw 'The Phase 8 archive already exists; inspect it before replacing it.' }
...
$receipt=@{archive='RelicVale-v0.8-baseline.zip';verified_files=$verified;bytes=(Get-Item -LiteralPath $archivePath).Length;sha256=(Get-FileHash -LiteralPath $archivePath -Algorithm SHA256).Hash;user_saves_included=$false}
